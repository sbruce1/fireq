...
K mmuc(K x, K y, K dxr, K dxc, K dyr, K dyc){
	...
	// Copy host arrays to device (column-major order)
	...
	// Build q matrix as list of column vectors
	K mat = ktn(0, cols);   // general list of length = number of columns
	for (J c = 0; c < cols; c++) {
		K col = ktn(KF, rows);  // each column is a float vector
		for (J r = 0; r < rows; r++) {
		    kF(col)[r] = kF(h_C)[c*rows + r]; // column-major indexing
		}
		kK(mat)[c] = col;
	}
